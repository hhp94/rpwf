...
## Demonstration
* Let's demonstrate using the `sim_classification()` function from the package 
`{modeldata}`.  
```{r}
set.seed(123)
df <- modeldata::sim_classification(num_samples = 500, num_linear = 20)
df[1:6, 1:6]
```
...
of how fitting models in python can have some very useful settings.  
  ...
  small.  
* Let's set up one a *radial basis kernel* svm model.  
  + I fixed the `kernel` argument in `args` to `rbf` to avoid confusion. 
  + This is because `{tidymodels}` defines `svm_poly()` and `svm_rbf()`  
  separately while sci-kit learn define them both in one class `sklearn.svm.SVC`.  
...
svm_rbf_model <- svm_rbf(cost = tune(),
  ...
  set_py_engine(
    ...
    "svm_rbf",
    tag = "svm_rbf",
    args = list(kernel = "rbf", # fix kernel parameter = "rbf"
                cache_size = 500)
  )
```

### `glm`
* We also fit an elastic net model.

```{r}
enet_model <- logistic_reg(penalty = tune(),
                           mixture = tune()) |>
  set_engine("glmnet") |>
  set_mode("classification") |>
  set_py_engine(
...
* Updating the range of the hyper parameter space is similar to how it works in
{dials}. Just provide the tuning function to the `.model_update_params` argument.  
...
### `xgboost`
* For the `xgboost_model`, I am using a `dials::grid_latin_hypercube`.  
* I want to limit the `max_depth` argument because this can cause memory issues.
To do this, I add a named list to the `.model_update_params` argument.  
```{r}
xgb_model <- xgb_model |>
  set_r_grid(
...
### `glm`
* I also use a 2D regular grid for our enet. However, I change the range of 
penalty to allows for a greater regularization strength.  
```{r}
...
* Recipes are defined just as a normal `tidymodels` workflow.  
  + Use formula or the role interface to specify the response and predictors.  
...
and one for `svm` and `glm` and `rbind()` them.   
...
### `rpwf_augment()`
* Add necessary information with `rpwf_augment()`  
* The `mtry` positive integer in R is sklearn's `colsample_bytree` positive 
proportions.  
* The `penalty` proportions in R is reciprocated in sklearn. This is why in 
order to have the l1 penalty > 1 (which is too large most of the time), I have 
to change the range upper bound to be smaller than 1.  
```{r}
