...
rpwf_add_model_info = function(obj, con) {
  ...
  rpwf = obj
  # Add the r_engine and the module where the base learner comes from
  rpwf$engine = vapply(rpwf$models, \(x) {x$engine}, "character")
  rpwf$py_module = vapply(rpwf$models, \(x) {x$py_module}, "character")
  rpwf$py_base_learner = vapply(rpwf$models, \(x) {x$py_base_learner}, "character")

  # Check for invalid models
  purrr::pwalk(.l = list(
    x = rpwf$py_module, y = rpwf$py_base_learner, z = rpwf$engine),
    .f = \(x, y, z) {rpwf_chk_model_avail(con, x, y, z)})
...
#'
#' @param val value vector to search the tables by
...
#' @return a vector of query values
#' @export
rpwf_query2 = function(val1, val2, query, con) {
...
#'
#' Add the [TrainDf()] R6 objects using provided recipes. Similar to add_grids,
...
#'
#' @param obj an object generated by [rpwf_add_cost()]
#' @inheritParams rpwf_dm_obj
#'
...
rpwf_add_model_type = function(obj, con) {
  ...
  query_res = rpwf_query2(
    obj$py_module, obj$py_base_learner,
    query =
...
#'
#' @param obj an object generated by [rpwf_add_model_type]
...
#' @inheritParams rpwf_dm_obj
#' @param proj_path root path of the project
...
#'
#' @examples
rpwf_add_all = function(wflow_obj, con, proj_path, .grid_fun = NULL,
                        ..., range = c(1L, 5000L), seed = 1234L) {
  set.seed(seed)
  wflow_obj |>
    rpwf_add_model_info(con = con) |>
    rpwf_add_desc() |>
    rpwf_add_grid_param(.grid_fun = .grid_fun, seed = seed, ...) |>
    rpwf_add_grids(con = con, proj_root_path = proj_path) |>
    rpwf_add_dfs(con = con,
                 proj_root_path = proj_path,
                 seed = seed) |>
    rpwf_add_cost(con = con) |>
    rpwf_add_model_type(con = con) |>
    rpwf_add_random_state(range = range, seed = seed)
}
