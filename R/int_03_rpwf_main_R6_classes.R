...
#' @param rename_fun a function to rename the grid, generated by
#' `rpwf_grid_rename_()`
#' @param grid_fun a `dials::grid_<functions>`, e.g., [dials::grid_random()],
#' [dials::grid_latin_hypercube()]. Default `NULL` assumes that no grid search is
#' performed and sklearn defaults are used.
...
rpwf_grid_gen_ <- function(model,
  ...
  # the arguments here.
  if (!is.null(model$grid_fun)) {
    fun <- model$grid_fun
  } else {
    fun <- grid_fun
  }

  if (!is.null(model$hyper_par_fun)) {
    update_list <- model$hyper_par_fun
  } else {
    update_list <- update_params
  }

  if (!is.null(model$grid_fun_args)) {
    grid_args <- model$grid_fun_args
  } else {
    grid_args <- list(...)
  }
  ...
  return(
    rpwf_transform_grid_(r_grid, rename_fun = rename_fun, n_predictors = params$n_predictors)
  )
}
...
#' @noRd
rpwf_transform_grid_ <- function(grid, rename_fun, n_predictors, ...) {
  r_grid <- dplyr::rename_with(grid, rename_fun)
...
#'
#' @examples
#' rename_fun <- rpwf_grid_rename_(jsonlite::toJSON(list("mtry" = "colsample_bytree")))
#' rename_fun(c("mtry", "not_added"))
