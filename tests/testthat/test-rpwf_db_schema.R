test_that("Can create a connection and create SQL tables", {
  ...
  created_tables = sort(DBI::dbListTables(con))
...
test_that("Duplicated values of model_type_tbl and cost_tbl are ignored", {
  ...
  before = sapply(c("cost_tbl", "model_type_tbl", "r_grid_tbl"),
                  \(x){nrow(DBI::dbGetQuery(con, glue::glue("SELECT * FROM {x}")))})
  # Try to add repeated values
  print(before)
  rpwf_db_ini_val(con = con)
  after = sapply(c("cost_tbl", "model_type_tbl", "r_grid_tbl"),
                 \(x){nrow(DBI::dbGetQuery(con, glue::glue("SELECT * FROM {x}")))})
  print(after)
