...
rpwf_Rgrid_R6_ <- function(obj, db_con) {
  obj$Rgrid <- lapply(obj$grids, \(x) {RGrid$new(x, db_con)})
  return(obj)
}
...
rpwf_TrainDf_R6_ <- function(obj, db_con) {
  obj$TrainDf <- lapply(obj$preprocs, \(x) {TrainDf$new(x, db_con)})
  return(obj)
}
...
rpwf_parquet_id_ <- function(obj, db_con) {
  path_chk_ <- function(R6_obj, root = db_con$proj_root_path) {
    sapply(R6_obj, \(x) { file.exists(paste(root, x$path, sep = "/")) })
  }
  #### Get grid id
