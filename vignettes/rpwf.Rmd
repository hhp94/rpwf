...
imputation and factor that uncertainty into the predictive performance 
...
* `set_py_engine()` has 3 important arguments  
  ...
  + `args` defines the arguments that can be passed to the base learner in python.  
    ...
    setting monotonicity contraints and different base learners such as linear 
...
```
  + For `svm_poly_model`, I use a grid regular with 100 levels, but only 2 
  polynomial degrees of 2 and 3.  
```{r}
svm_poly_model <- svm_poly_model |>
  set_r_grid(dials::grid_regular, levels = 9, filter = degree %in% c(2, 3))
```
  + For `svm_rbf_model`, I just use a 2D regular grid with 100 levels.  
```{r}
...
* I added one special role that can be used with the `update_role()` function:
  + `pd.index`
```{r}
...
## Create workflowset and export the into parquets and database.  
...
Custom functions are possible but would require coding on the python side.    

```{r}
xgb_wfs <- rpwf_workflow_set(
  list(xgb_base = xgb_base_rec, xgb_pca = xgb_pca_rec),
  list(xgb = xgb_model),
  cost = "roc_auc"
)

svm_wfs <- rpwf_workflow_set(
  list(svm_base = svm_base_rec, svm_pca = svm_pca_rec),
  list(svm_poly = svm_poly_model, svm_rbf = svm_rbf_model),
  cost = "roc_auc"
)
