...
#' Change a Column Role to "pd.index"
#'
#' {rpwf} reserves "pd.index" as a special role for pandas index. This function
#' is a wrapper for [recipes::update_role()] where the new role is a "pd.index"
#'
#' @param obj recipe object
#' @param index_col index column
#'
#' @return recipe object with a variable that has the "pd.index" role.
#' @export
#'
#' @examples
...
#' r
update_role_pd_index <- function(obj, index_col) {
  stopifnot(methods::is(obj, "recipe"))
  recipes::update_role(obj, {{index_col}}, new_role = "pd.index")
}
